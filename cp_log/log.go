...
func NewLogger(logConf *cp_dc.DcLogConfig) *Logger {
	...
	if logConf.ConsoleWriter.Enable {
		...
		case cp_constant.LevelPanic:
			level = zapcore.DebugLevel
		case cp_constant.LevelFatal:
			level = zapcore.FatalLevel
		case cp_constant.LevelError:
			level = zapcore.ErrorLevel
		case cp_constant.LevelWarning:
			level = zapcore.WarnLevel
		case cp_constant.LevelInformational:
			level = zapcore.InfoLevel
		case cp_constant.LevelDebug:
			level = zapcore.DebugLevel

		default:
			panic(errors.New("logConf.ConsoleWriter.level 非法:" + logConf.ConsoleWriter.Level))
		}
	...
	if logConf.InfoFileWriter.Enable {
		...
		default:
			panic(errors.New("logConf.InfoFileWriter.level 非法:" + logConf.InfoFileWriter.Level))
		}
	...
	if logConf.ErrorFileWriter.Enable {
		...
		default:
			panic(errors.New("logConf.ErrorFileWriter.level 非法:" + logConf.ErrorFileWriter.Level))
		}
